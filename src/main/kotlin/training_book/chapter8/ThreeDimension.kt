...
fun main() = PApplet.main("training_book.chapter8.ThreeDimension")

class ThreeDimension : PApplet() {
    val particles: MutableList<ParticleVector3> = mutableListOf()
    // 時間発展する軌道を引き付ける空間領域
    val attractors: MutableList<ParticleVector3> = mutableListOf()
    ...
    override fun draw() {
        ...
        // アトラクターを描画
        attractors.forEach { attract ->
            fill(255f, 0f, 0f)
            attract.draw()
            particles.forEach { particle ->
                // attractへ吸引する力を計算
                particle.attract(attract.location, 50f, 10f, 800f)
            }
        }
        particles.forEach {
            it.update()
